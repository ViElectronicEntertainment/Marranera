 ...
 */ {
	class mthumb {
		...
		public function __construct() {
			global $ALLOWED_SITES, $ALLOW_ANY;
			...
			// end mindshare fix for tilde's
                        if (strpos($this->src, 'https') !== false) {
                                if (strpos($this->src, ':443') !== false) {
                                        $this->src=str_replace(":443","",$this->src);
                                }
                        }
			...
			if ($this->isURL) {
				if($ALLOW_ANY){
					...
					foreach ($ALLOWED_SITES as $site) {
						if ((strtolower(substr($this->url[ 'host' ], -strlen($site) - 1)) === strtolower(".$site")) || (strtolower($this->url[ 'host' ]) === strtolower($site)) || strpos(strtolower($this->url[ 'host' ]), strtolower($site)) !== false) {
		...
		protected function serveExternalImage() {
			...
			if (!preg_match("/^image\/(?:jpg|jpeg|gif|png)$/i", $mimeType)) {
				...
				@unlink($this->cachefile);
				touch($this->cachefile);
				$this->error($tempfile."The remote file is not a valid image. Mimetype = '" . $mimeType . "'" . $tempfile);
